<template>
  <div class="fasilitas-container">
    ...
    <div class="right">
      ...
      <p v-if="operationalHours.open && operationalHours.close">
        {{ formatTime(operationalHours.open) }} -
        {{ formatTime(operationalHours.close) }}
      </p>
      <p v-else>Memuat...</p>
...
const handlePesan = () => {
  ...
  if (!userId) {
    alert("Silakan login terlebih dahulu untuk membuat pemesanan!");
    router.push("/signin");
    return;
  }

  router.push(`/bookingpage/${props.spot.id}/${props.spot.slug}`);
};
...
const loadNextSession = async () => {
  try {
    const res = await axios.get(
      `/api/sessions/${props.spot.id}/next-session`
    );
...
// ===========================
// LOAD OPERASIONAL HOURS
// ===========================
const loadOperationalHours = async () => {
  try {
    const res = await axios.get(
      `/api/sessions/${props.spot.id}/operational-hours`
    );
    console.log("🔵 RESPONSE HOURS:", res.data);

    operationalHours.value = res.data;
  } catch (err) {
    console.error("❌ ERROR HOURS:", err);
  }
};
